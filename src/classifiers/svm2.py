...
class SVM(object):
    ...
    def __init__(self, kernel_type, grau=None, C=0, limite=1e-5, **parametros):
        
        self.kernel_type = kernel_type
        if kernel_type == 'linear':
            self.kernel = kernel_linear
        elif kernel_type == 'polynomial':
            self.kernel = kernel_polinomial
        self.grau = grau
        self.parametros = parametros
        self.C = np.float(C)
        self.limite = limite

    def svm_kernel_fit(self, X, kernel_type, grau = None):
        # todas as observações de treino
        if kernel_type == 'linear':
            return np.matmul(X, X.T)
        elif kernel_type == 'polynomial':
            return (np.matmul(X, X.T)+1)**grau
    ...
    def fit(self, X, y):
        ...
        K = self.svm_kernel_fit(X, self.kernel_type, self.grau)
...
def teste():
    ...
    svm_clf = SVM(kernel_type='linear', kernel=kernel_linear)
    svm_clf.fit(X, y_and)
    ...
    svm_clf = SVM(kernel_type='polynomial', kernel=kernel_polinomial, grau=2, escalar=1)
    svm_clf.fit(X, y_and)
    y_hat = svm_clf.predict(X)
    assert((y_hat == y_and).all())
    # print(y_and)
